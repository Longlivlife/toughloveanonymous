...
export async function POST(request: NextRequest) {
  try {
    ...
    if (testimony_id) {
      const { data: existing } = await supabase.from('testimonies').select('raw_transcript').eq('id', testimony_id).eq('survivor_id', user.id).single()
      if (existing) {
        const appended = (existing.raw_transcript || '') + '\n\n---\n\n' + transcript
        const { error } = await supabase.from('testimonies').update({ raw_transcript: appended }).eq('id', testimony_id)
        if (error) return NextResponse.json({ error: 'Failed to update testimony' }, { status: 500 })
        return NextResponse.json({ ok: true, testimony_id })
      }
    }

    const { data, error } = await supabase.from('testimonies').insert({
      survivor_id: user.id, raw_transcript: transcript,
